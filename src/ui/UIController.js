...
export class UIController {
    ...
    setupControlPanel() {
        ...
        const echoBtn = document.getElementById('show-echo');
        ...
        if (echoBtn) {
            echoBtn.addEventListener('click', () => {
                toggleButton(echoBtn, (newState) => {
                    console.log('[ECHO] Button state:', newState);

                    if (newState) {
                        // Echo ON → call runEcho from App (main.js)
                        if (window.app && window.app.runEcho) {
                            window.app.runEcho();
                        } else {
                            console.warn('App.runEcho not available');
                        }
                    } else {
                // Echo OFF → reset to neutral robot
                        console.log('[UI] Echo OFF');
                        window.app?.resetToNeutral();
                    }
                });
            });
        }
