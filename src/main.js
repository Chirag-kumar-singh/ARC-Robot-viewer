...
class App {
    ...
    async init() {
        ...
        try {
            ...
            if (!config) {
                ...
                if (this.currentModel.links.has(linkName)) {
                    ...
                    link.visuals.forEach(v => {
                        const obj = v.threeObject;

                        obj.traverse(child => {
                            if (child.isMesh) {
                                // IMPORTANT: clone material to avoid shared-material issues
                                child.material = child.material.clone();
                                child.material.color.setRGB(1, 0, 0); // RED
                                child.material.needsUpdate = true;

                                console.log('[JS COLOR TEST] FL_hip colored RED');
                            }
                        });
                    });
                } else {
                    console.warn('[JS COLOR TEST] Link not found:', linkName);
                }

            } catch (err) {
                console.error('[AUTOLOAD] Failed to load Go2:', err);
            }
            ...
            //             // 🔽 AUTO-LOAD Go2 URDF FROM public/
            // (async () => {
            // try {
            //     const ROOT = '/go2_description-humble/';
            //     const URDF_URL = '/go2_description-humble/urdf/go2_description.urdf';

            //     console.log('[AUTOLOAD] Fetching Go2 URDF');

            //     // Fetch URDF text (same as folder upload does internally)
            ...
            //     // Build virtual file map (minimal – URDF only)
            //     const fileMap = new Map();
            //     fileMap.set('urdf/go2_description.urdf', urdfText);

            //     // IMPORTANT: use the same internal method as "Load Folder"
            //     await this.fileHandler.loadURDFFromText(
            //     urdfText,
     ...
     runEcho() {
        ...
        const modifiedUrdf = this.buildActiveURDF(
            this.TEMPLATE_URDF_TEXT,
            activeLinks
        );

        this.loadURDFFromText(modifiedUrdf);
    }

    buildActiveURDF(templateUrdfText, activeLinks) {
        let urdf = templateUrdfText;

        activeLinks.forEach(linkName => {
            const linkBlockRegex = new RegExp(
                `<link\\s+name="${linkName}"[\\s\\S]*?<\\/link>`,
                'm'
            );

            const match = urdf.match(linkBlockRegex);
            if (!match) return;

            const linkBlock = match[0];

            const updatedLinkBlock = linkBlock.replace(
                /filename="([^"]+)\.dae"/g,
                'filename="$1 copy.dae"'
            );

            urdf = urdf.replace(linkBlock, updatedLinkBlock);
        });

        return urdf;
    }

    async loadURDFFromText(urdfText) {
        console.log('[URDF] Reloading robot');
        ...
        console.log('[URDF] Robot reloaded');
    }
...
// 🔒 Force all floating panels closed at startup
[
